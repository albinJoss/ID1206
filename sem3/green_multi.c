...
//Change the original code to be usuable across multiple CPU cores and implement actual parallelism instead of concurrency across one logical CPU thread. This is an something extra which Johan suggested could be done.
...
void green_semaphore_init(green_semaphore_t *semaphore, int value)
{
    semaphore->value = value;
    green_cond_init(&semaphore->cond);
    green_mutex_init(&semaphore->mutex);
}

void green_semaphore_wait(green_semaphore_t *semaphore)
{
    green_mutex_lock(&semaphore->mutex); //Lock the mutex
    while (semaphore->value <= 0)
    {
        green_cond_wait(&semaphore->cond, &semaphore->mutex); //Wait until the value is more than 0 then the thread can continue with execution.
    }
    semaphore->value--; //Decrement the value to signal to the other semaphores in use that they can't continue at this time and that this semaphore will be the one to continue
    green_mutex_unlock(&semaphore->mutex);
}

void green_semaphore_post(green_semaphore_t *semaphore)
{
    green_mutex_lock(&semaphore->mutex);
    semaphore->value++;
    green_cond_signal(&semaphore->cond);
    green_mutex_unlock(&semaphore->mutex);
}